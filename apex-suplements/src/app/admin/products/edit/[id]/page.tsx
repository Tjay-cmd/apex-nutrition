...
import { getProduct, updateProduct, uploadBase64ToStorage } from '@/lib/firebase-queries';
import { compressImage, isImageSizeAcceptable } from '@/lib/image-utils';
import type { UpdateProductData } from '@/types/product';

interface ProductForm extends UpdateProductData {
  // Extends the UpdateProductData interface
}
...
const EditProductPage = () => {
      ...
      try {
        ...
        if (product) {
          setFormData({
            ...
            back_label_image_url: product.back_label_image_url || '',
          ...
          if (product.back_label_image_url) {
            setBackLabelImagePreview(product.back_label_image_url);
          }

          // Set text fields for ingredients and benefits
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Upload images to Firebase Storage if they are new base64 data
      ...
      // Update product data with storage URLs
      const productData = {
        ...formData,
        image_url: imageUrl,
